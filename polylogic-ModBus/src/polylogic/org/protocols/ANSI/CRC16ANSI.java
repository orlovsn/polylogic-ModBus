 ...
 */
package polylogic.org.protocols.ANSI;

/**
 * This class performs CRC16ANSI calculations and checks. CRC16 is widely used
 * in many protocols - it has only two bytes overhead and fast calculation,
 * allowing not only fail-safe data transmission, but also allowing multiple
 * devices with different protocols and baud rates to operate via single line.
 * ANSI version is most common used one, it is used in Modbus RTU, Modbus over
 * TCP and many other protocols, like Polyservice KX-EX protocol.
 *
...
public class CRC16ANSI {
    ...
    public static boolean checkCRC(byte[] reply) {
        if (reply == null || reply.length < 3) {
            // minimal content is value+crc1+crc2
            return false;
        } else {
